...
pub fn handler<'info>(
    ...
    require!(
        game.bets.len() < MAX_BETS_PER_GAME,
    ...
    let wallet_index =
        if let Some(index) = game.wallets.iter().position(|&wallet| wallet == user.key()) {
            index as u16
        } else {
            // New user - add to wallets and increment user_count
            game.wallets.push(user.key());
            game.user_count = game
                .user_count
                .checked_add(1)
                .ok_or(Domin8Error::ArithmeticError)?;

            (game.wallets.len() - 1) as u16
        };

    // Check if user has exceeded maximum bets per game based on bet amount
    let user_bet_count = game
        .bets
        .iter()
    ...
    // Add the bet to game WITH skin and position
    game.bets.push(BetInfo {
        wallet_index,
        amount: bet_amount,
        skin,
        position,
    ...
    // Update game total_deposit
    game.total_deposit = game
        .total_deposit
    ...
    msg!("Total pot: {} lamports", game.total_deposit);
